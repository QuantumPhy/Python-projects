...
def print_learning_learned_comparison(X, Y, losses, bookranges, batch_loss, batch_accuracy, epoch_size, index, epoch):
    ...
    # ┌ \u250C
    # ┐ \u2510
    format_string = "{:0^" + str(len(epoch_string)) + "}"
    format_string += "{:1^" + str(len(formatted_bookname)) + "}"
    format_string += "{:2^" + str(len(decx) + 2) + "}"
    format_string += "{:3^" + str(len(decy) + 2) + "}"
    format_string += "{:4^" + str(len(loss_string)) + "}"
